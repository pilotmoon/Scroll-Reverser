...
product_nospaces=`echo "$PRODUCT_NAME" | tr -d ' '`
zipname="$product_nospaces-$version.zip"
folder=$BUILT_PRODUCTS_DIR
appname=$FULL_PRODUCT_NAME
product_lowercase=`echo $product_nospaces | tr '[A-Z]' '[a-z]'`
...
zip --quiet -r -y "$zipname" "$appname"

# create appcast for release builds
if [ ${CONFIGURATION} == 'Release' ]
then

# get values for appcast
echo "Configuration is Release. Creating appcast entry."

# get path to key (NM_SPARKLE_KEYS_FOLDER should be set globally)
key="$NM_SPARKLE_KEYS_FOLDER/${product_nospaces}_dsa_priv.pem"
